...
export default function FormularioOferta({ oferta, onClose, onSuccess }) {
  ...
  const isEmpresa = String(user?.tipo_usuario || user?.rol || user?.role || '')
    .toLowerCase() === 'empresa';
  // Tema naranja solo cuando la empresa crea (no edita) oferta desde su dashboard
  const usarTemaEmpresa = isEmpresa && !esEdicion;
  ...
  // === Render condicional según contexto ===
  if (usarTemaEmpresa) {
    return (
      <div className="fixed inset-0 bg-black/50 backdrop-blur-sm flex items-center justify-center z-50 p-4">
        <div className="bg-gradient-to-br from-orange-50 via-amber-50 to-yellow-50 rounded-xl shadow-2xl max-w-5xl w-full max-h-[92vh] flex flex-col border border-orange-200">
          {/* Header Tema Empresa (CREAR) */}
          <div className="relative bg-gradient-to-r from-orange-700 via-orange-600 to-amber-600 text-white p-6 rounded-t-xl flex-shrink-0">
          <button
            onClick={onClose}
          ...
          <div className="flex items-start gap-4 pr-12">
            ...
            <div className="flex-1 min-w-0">
              <div className="flex flex-wrap items-center gap-2 mb-2">
                {oferta ? (
                  <span className={`px-3 py-1.5 rounded-lg text-xs font-bold flex items-center gap-1.5 shadow-md ${formData.estado_oferta === 'activa' ? 'bg-emerald-500 text-white' : formData.estado_oferta === 'pausada' ? 'bg-amber-500 text-white' : 'bg-red-600 text-white'}`}>
                    <span>{getEstadoIcon(formData.estado_oferta)}</span>
                    <span className="capitalize">{formData.estado_oferta}</span>
                  </span>
                ) : (
                  <span className="px-3 py-1.5 bg-white text-orange-700 rounded-lg text-xs font-bold shadow-md">
                    Nueva Oferta
                  </span>
                )}
                <span className="px-3 py-1.5 bg-white text-orange-700 rounded-lg text-xs font-bold shadow-md capitalize">
              ...
              <p className="text-orange-100 flex items-center gap-1.5 text-sm">
                <span>🏢</span>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 mb-6">
            <div className="bg-white p-4 rounded-lg border border-orange-200 shadow-sm">
              <div className="flex items-center gap-2 mb-2">
                ...
                <span className="text-xs text-orange-700 font-semibold uppercase">Cupos</span>
              </div>
            ...
            <div className="bg-white p-4 rounded-lg border border-orange-200 shadow-sm">
              <div className="flex items-center gap-2 mb-2">
                <span className="text-2xl">⏰</span>
                <span className="text-xs text-orange-700 font-semibold uppercase">Duración</span>
              </div>
            ...
            <div className="bg-white p-4 rounded-lg border border-orange-200 shadow-sm">
              <div className="flex items-center gap-2 mb-2">
                <span className="text-2xl">💰</span>
                <span className="text-xs text-orange-700 font-semibold uppercase">Salario ref.</span>
              </div>
          ...
          <section className="mb-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div className={`p-4 bg-white rounded-lg border ${errors.id_empresa ? 'border-red-400' : 'border-orange-200'} shadow-sm`}>
                <label className="block text-xs text-orange-700 font-semibold uppercase mb-2">
                  Empresa <span className="text-red-500">*</span>
                </label>
                {esEdicion ? (
                  <>
                    <div className="w-full px-3 py-2 border border-orange-200 rounded-lg bg-orange-50 text-gray-800 font-semibold cursor-not-allowed">
                  ...
                  <div className="w-full px-3 py-2 border border-orange-200 rounded-lg bg-orange-50 text-gray-800 font-semibold">
                    {miEmpresa?.nombre_comercial
                      || empresas.find((e) => String(e.id_empresa) === String(formData.id_empresa))?.nombre_comercial
                      || empresas.find((e) => String(e.id_empresa) === String(formData.id_empresa))?.razon_social
                      || 'Empresa no disponible'}
              ...
              <div className={`p-4 bg-white rounded-lg border ${errors.id_especialidad ? 'border-red-400' : 'border-orange-200'} shadow-sm`}>
                <label className="block text-xs text-orange-700 font-semibold uppercase mb-2">
              ...
              <div className={`md:col-span-2 p-4 bg-white rounded-lg border ${errors.titulo_oferta ? 'border-red-400' : 'border-orange-200'} shadow-sm`}>
                <label className="block text-xs text-orange-700 font-semibold uppercase mb-2">
          ...
          <section className="mb-6">
            ...
            <div className="grid grid-cols-1 gap-4">
              <div className={`p-4 bg-white rounded-lg border ${errors.descripcion ? 'border-red-400' : 'border-orange-200'} shadow-sm`}>
                <label className="block text-xs text-orange-700 font-semibold uppercase mb-2">
              ...
              <div className="p-4 bg-white rounded-lg border border-orange-200 shadow-sm">
                <label className="block text-xs text-orange-700 font-semibold uppercase mb-2">Requisitos</label>
                <textarea
                  name="requisitos"
          ...
          <section className="mb-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div className={`flex items-center gap-3 p-4 bg-white rounded-lg border ${errors.duracion_horas ? 'border-red-400' : 'border-orange-200'} shadow-sm`}>
                <div className="w-12 h-12 bg-orange-50 rounded-lg flex items-center justify-center flex-shrink-0">
                  <span className="text-2xl">⏰</span>
                </div>
                <div className="min-w-0 flex-1">
                  <p className="text-xs text-orange-700 font-semibold uppercase mb-1">Duración (horas) *</p>
                  <input
                    type="number"
                    name="duracion_horas"
              ...
              <div className={`flex items-center gap-3 p-4 bg-white rounded-lg border ${errors.cupos_disponibles ? 'border-red-400' : 'border-orange-200'} shadow-sm`}>
                <div className="w-12 h-12 bg-orange-50 rounded-lg flex items-center justify-center flex-shrink-0">
                  <span className="text-2xl">🎯</span>
                </div>
                <div className="min-w-0 flex-1">
                  <p className="text-xs text-orange-700 font-semibold uppercase mb-1">Cupos Disponibles *</p>
                  <input
                    type="number"
              ...
              <div className="flex items-center gap-3 p-4 bg-white rounded-lg border border-orange-200 shadow-sm">
                <div className="w-12 h-12 bg-orange-50 rounded-lg flex items-center justify-center flex-shrink-0">
                  <span className="text-2xl">🕐</span>
                </div>
                <div className="min-w-0 flex-1">
                  <p className="text-xs text-orange-700 font-semibold uppercase mb-1">Horario de Trabajo</p>
                  <input
                    type="text"
              ...
              <div className="flex items-center gap-3 p-4 bg-white rounded-lg border border-orange-200 shadow-sm">
                ...
                <div className="min-w-0 flex-1">
                  <p className="text-xs text-orange-700 font-semibold uppercase mb-1">Ubicación</p>
                  <input
                    type="text"
                    name="ubicacion"
              ...
              <div className="flex items-center gap-3 p-4 bg-white rounded-lg border border-orange-200 shadow-sm">
                ...
                <div className="min-w-0 flex-1">
                  <p className="text-xs text-orange-700 font-semibold uppercase mb-1">Modalidad *</p>
                  <select
                    ...
                    onChange={handleChange}
                    required
                    className="w-full px-3 py-2 border border-orange-300 rounded-lg focus:ring-2 focus:ring-orange-500 capitalize"
                  >
              ...
              <div className="flex items-center gap-3 p-4 bg-white rounded-lg border border-orange-200 shadow-sm">
                ...
                <div className="min-w-0 flex-1">
                  <p className="text-xs text-orange-700 font-semibold uppercase mb-1">Salario Referencial (opcional)</p>
                  <input
                    type="number"
              ...
              <div className={`flex items-center gap-3 p-4 bg-white rounded-lg border ${errors.fecha_limite_postulacion ? 'border-red-400' : 'border-orange-200'} shadow-sm`}>
                <div className="w-12 h-12 bg-orange-50 rounded-lg flex items-center justify-center flex-shrink-0">
                  <span className="text-2xl">📅</span>
                </div>
                <div className="min-w-0 flex-1">
                  <p className="text-xs text-orange-700 font-semibold uppercase mb-1">Fecha Límite Postulación *</p>
                  <input
                    type="date"
              ...
              <div className={`flex items-center gap-3 p-4 bg-white rounded-lg border ${errors.fecha_inicio ? 'border-red-400' : 'border-orange-200'} shadow-sm`}>
                <div className="w-12 h-12 bg-orange-50 rounded-lg flex items-center justify-center flex-shrink-0">
                  <span className="text-2xl">⏳</span>
                </div>
                <div className="min-w-0 flex-1">
                  <p className="text-xs text-orange-700 font-semibold uppercase mb-1">Fecha de Inicio *</p>
                  <input
                    type="date"
                    name="fecha_inicio"
          ...
          <section className="mb-2">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div className="p-4 bg-white rounded-lg border border-orange-200 shadow-sm md:col-span-2">
                <label className="block text-xs text-orange-700 font-semibold uppercase mb-2">Beneficios</label>
                <textarea
                  name="beneficios"
              ...
              {oferta && (
                <div className="p-4 bg-white rounded-lg border border-orange-200 shadow-sm">
                  <label className="block text-xs text-orange-700 font-semibold uppercase mb-2">Estado</label>
                  <select
                    name="estado_oferta"
          ...
          <div className="border-t border-orange-200 bg-white px-6 py-4 rounded-b-xl flex-shrink-0">
          <div className="flex flex-col sm:flex-row gap-3">
            <button
              type="button"
              onClick={onClose}
              disabled={loading}
              className="flex-1 px-4 py-2.5 border border-orange-300 text-orange-700 rounded-lg hover:bg-orange-50 transition-colors font-semibold disabled:opacity-50"
            >
              Cancelar
            </button>
            <button
              ...
              disabled={loading}
              className="flex-1 px-4 py-2.5 bg-gradient-to-r from-orange-600 to-amber-600 text-white rounded-lg hover:from-orange-700 hover:to-amber-700 transition-colors font-semibold flex items-center justify-center gap-2 shadow-md disabled:opacity-50"
            >
              {loading ? (
                <>
  ...
  // === Tema gris original (admin y edición) ===
  return (
    <div className="fixed inset-0 bg-gray-900/60 backdrop-blur-sm flex items-center justify-center z-50 p-4">
